...
/** @defgroup DMC_Macros Macros
  @{
*/

/**
 * @brief Configure the DMC RD Clock Delay
 */
#if defined(APM32F407xx) || defined(APM32F417xx)
#define DMC_SET_RD_DELAY(__DELAY__)         (DMC->CTRL2_B.RDDCFG = (__DELAY__))
#else
#define DMC_SET_RD_DELAY(__DELAY__)                         \
    do {                                                    \
        DMC->CTRL2_B.RDDCFG1 = ((__DELAY__) & 0x07);        \
        DMC->CTRL2_B.RDDCFG2 = (((__DELAY__) & 0x18) >> 3); \
       } while(0)
#endif /* APM32F407xx || APM32F417xx */

/**@} end of group DMC_Macros*/
...
/* Refresh mode */
void DMC_ExitSlefRefreshMode(void);
