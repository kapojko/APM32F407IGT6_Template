...
typedef enum
{
    ...
    ETH_MMC_INT_RGUF   = BIT17 | BIT28, /*!< When Rx good unicast frames counter reaches half the maximum value */
    ETH_MMC_INT_RFAE   = BIT6 | BIT28,  /*!< When Rx alignment error counter reaches half the maximum value */
    ETH_MMC_INT_RFCE   = BIT5 | BIT28   /*!< When Rx crc error counter reaches half the maximum value */
} ETH_MMC_INT_T;
...
#define ETH_DMARXDESC_RXWDTTO     0x00000010U   /*!< Receive Watchdog Timeout: watchdog timer expired during reception */
...
uint8_t ETH_ReadDMATxDescFlagStatus(ETH_DMADescConfig_T *DMATxDesc, uint32_t flag);
...
uint8_t ETH_ReadDMARxDescFlagStatus(ETH_DMADescConfig_T *DMARxDesc, uint32_t flag);
#if USE_ENHANCED_DMA_DESCRIPTORS
uint8_t ETH_ReadDMAPTPRxDescExtendedFlagStatus(ETH_DMADescConfig_T *DMAPTPRxDesc, uint32_t flag);
#endif
