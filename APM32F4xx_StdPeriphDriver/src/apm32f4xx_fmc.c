 ...
 *            @arg FMC_LTNCY_8 - FMC_LTNCY_15: FMC 8 - 15 Latency cycles (*)
 *            @arg FMC_LTNCY_16 - FMC_LTNCY_127: FMC 16 - 127 Latency cycles (*)
 *
 * @retval    None
 */
...
{
    FMC->ACCTRL_B.PREF_TIMING = ENABLE;
}
...
{
    FMC->ACCTRL_B.PREF_TIMING = DISABLE;
}

/*!
 * @brief     Enables the FLASH Read Interrupt.
 *
 * @param     None
 *
 * @retval    None
...
{
    FMC->ACCTRL_B.RD_INT_EN = ENABLE;
}

/*!
 * @brief     Disables the FLASH Read Interrupt.
 *
 * @param     None
 *
 * @retval    None
...
{
    FMC->ACCTRL_B.RD_INT_EN = DISABLE;
}

/*!
 * @brief     Enables the FLASH Read Interrupt time
 *
 * @param     trcMode : FLASH Read Interrupt time
 *              @arg FMC_TRC_FAST: Fast flash read interrput cycle
 *              @arg FMC_TRC_SLOW: Slow flash read interrput cycle
 *
 * @retval    None
 */
void FMC_ConfigTrcMode(FMC_TRC_T trcMode)
{
    if (trcMode == FMC_TRC_FAST)
    {
        FMC->ACCTRL_B.RD_INT_TIMING = BIT_RESET;
    }
    else
    {
        FMC->ACCTRL_B.RD_INT_TIMING = BIT_SET;
    }
}
...
FMC_STATUS_T FMC_ReadStatus(void)
{
    ...
    else if ((FMC->STS & FMC_FLAG_ERRPGS) == FMC_FLAG_ERRPGS)
    {
        status = FMC_ERROR_PGS;
    }
    else if ((FMC->STS & FMC_FLAG_ERRPGP) == FMC_FLAG_ERRPGP)
    {
        status = FMC_ERROR_PGP;
    }
    else if ((FMC->STS & FMC_FLAG_ERRPGA) == FMC_FLAG_ERRPGA)
    {
        status = FMC_ERROR_PGA;
    }
    else if ((FMC->STS & FMC_FLAG_ERRWRP) == FMC_FLAG_ERRWRP)
    {
    ...
    else if ((FMC->STS & FMC_FLAG_ERROP) == FMC_FLAG_ERROP)
    {
